...
import type * as OldModel from "../version-39.0.0-pre.7/overview-model.js";
...
function transformNewFilterConfiguration(
	...
	const { filterGroups, ...rest } = oldConfig;

	return {
		...rest,
		filterGroups: filterGroups.map(({ collapsed: _collapsed, ...group }) => group)
	};
}
