...
export namespace OverviewModel {
	...
	export namespace ReferenceColumn {
		export function isAssignableFrom(column: object): column is ReferenceColumn {
			return "elementRef" in column && !LinkColumn.Reference.isAssignableFrom(column);
		}
	}
	...
	export namespace LinkColumn {
		export interface Reference
			extends Omit<ReferenceColumn, "sortable" | "preferredSorting" | "summary">, BaseLinkedColumn {}
		...
		export namespace Expression {
			export function isAssignableFrom(column: object): column is Expression {
				return ExpressionColumn.isAssignableFrom(column) && BaseLinkedColumn.isAssignableFrom(column);
			}
		}
	}
	...
	export interface NewFilterConfiguration {
		...
		/** Filter Bar area: 3-dot-menu options. Optional — omit to disable the Filter Bar entirely. */
		readonly filterBar?: NewFilter.FilterBarConfiguration;
	}
	...
	export namespace NewFilter {
		/** Logical group of filter items, rendered as a collapsible section in the Filter Selector. */
		export interface Group extends Options.Collapsed {
			readonly id: string;
		...
		export interface FilterSelectorConfiguration {
			...
			/** Global invert (NOT) applied to the combined selector-area result. */
			readonly invert: Configurable<boolean>;
			/** Global join operator (AND/OR) across all selector-area filters. */
		...
		/**
		 * Filter Bar area configuration. Bar-level join/invert toggles live directly on the wrapper.
		 * Mirrors `FilterState.filterBarOptions`. Per-subkey optionality.
		 */
		export interface FilterBarConfiguration {
			readonly joinOperator?: Configurable<"and" | "or">;
			readonly invert?: Configurable<boolean>;
		}
