...
const CustomTableBody: React.ComponentType<TableBody.Props> = (props) => {
	...
	if (data.length === 0) {
		return <div className="-u-flex -u-justify-center">No search results. Try again with another query</div>;
	}

	return <>{DefaultTableComponentRenderers.bodyRenderer(props)}</>;
};

export const CustomOverviewEngineContainer: React.ComponentType<OverviewEngine.Props> = (engineProps) => {
	return <OverviewEngine {...engineProps} componentMap={{ ...DefaultComponentMap, TableBody: CustomTableBody }} />;
};
