...
describe("com.mgmtp.a12.overview-engine.view.components.filters.optionsViews.boolean-filter-options-view", () => {
	const onChangeSpy = vi.fn();
	const viewName = "Boolean";
	const filterType = "Boolean";

	const basicProps: BooleanFilterOptionsView.Props = {
		path: [{ elementName: "root" }, { elementName: "boolean" }],
		viewName,
	...
	function setupTest(props?: Partial<BooleanFilterOptionsView.Props>, engineProps?: Partial<OverviewEngine.Props>) {
		return render(<BooleanFilterOptionsView {...basicProps} {...props} />, {
	...
	describe("onChange", () => {
		describe("when the user clicks the true radio button", () => {
			it("calls the given onChange callback with value: true", () => {
				...
				expect(onChangeSpy).toHaveBeenCalledExactlyOnceWith({ criteria: { value: true }, filterType });
			});
		});

		describe("when the user clicks the false radio button", () => {
			it("calls the given onChange callback with value: false", () => {
				const wrapper = setupTest();

				const radioButton = wrapper.getByLabelText(en.overviewEngine.filterOptionView.false).element;

				expect(radioButton).toBeInTheDocument();

				fireEvent.click(radioButton);

				expect(onChangeSpy).toHaveBeenCalledExactlyOnceWith({ criteria: { value: false }, filterType });
			});
		});
		...
		describe("when the user clicks the `null` radio button", () => {
			it("calls the given onChange callback with value: null", () => {
				...
				expect(onChangeSpy).toHaveBeenCalledExactlyOnceWith({ criteria: { value: null }, filterType });
			});
		});
	});
	...
	describe("clearAll", () => {
		it("call the onChange callback with empty criteria", () => {
			const clearAllButton = getClearAllButton(setupTest({ uiValue: { value: true } }));
	...
	describe("props", () => {
		...
		describe("filters", () => {
			...
			describe("given initialFilterOptions with value false", () => {
				it("selects the no radio item", () => {
					const wrapper = setupTest({ uiValue: { value: false } });

					expect(wrapper.getByLabelText(en.overviewEngine.filterOptionView.false).element).toBeChecked();
				});
			});

			describe("given initialFilterOptions with value null", () => {
				it("selects the `null` radio item", () => {
			...
			describe("given no initialFilterOptions", () => {
				it("doesn't select a radio item", () => {
					setupTest()
						.queryAllByDataRole(DataRoles.Radio.Input)
						.forEach((radio) => {
		...
		describe("disabled", () => {
			...
			describe("given false", () => {
				...
				describe("given a radio button is checked", () => {
		...
		it("renders radio options of Boolean field filter in the correct order: null, true, false", () => {
			const wrapper = setupTest();

			const radioInputs = wrapper.getAllByDataRole(DataRoles.Radio.Input);
			const expectedOrder = [
				en.overviewEngine.filterOptionView.null,
				en.overviewEngine.filterOptionView.true,
				en.overviewEngine.filterOptionView.false
			];
		...
		describe("hideEmptyValueOption", () => {
			it("omits the empty/null radio option when enabled", () => {
				...
				expect(wrapper.getByLabelText(en.overviewEngine.filterOptionView.false).element).toBe(radioInputs[1]);
			});
		});
	});
});
