...
export const onFilterItemEditApplied: Middleware<{}, UiState> = (api) => (next) => (action) => {
	const result = next(action);

	if (Events.NewFilter.onFilterItemEditApplied.match(action)) {
		...
		const { filterId, options: editingOptions } = state.editingFilter;
		const targetFilter = state.filters[filterId];

		if (!targetFilter) {
			return result;
		}

		const mergedOptions = { ...targetFilter.options, ...editingOptions };

		api.dispatch(
			Commands.setFilterState({
				state: pipe(
					state,
					FilterStateLens.filterOptionsById(filterId).set(mergedOptions),
					FilterStateLens.filterAppliedOptionsById(filterId).set(mergedOptions),
					FilterStateLens.editingFilter.set(null)
				)
			})
		);
