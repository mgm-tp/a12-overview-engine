 ...
 * @internal
 */
export const onFilterChanged: Middleware = (api) => (next) => (action) => {
	const result = next(action);

	if (Events.onFilterChanged.match(action)) {
		const currentQueryParameters = UiStateSelector.queryParameters()(api.getState());
		const nextQueryParameters = resetPaginationAndScrolling({
			currentQueryParameters,
			payload: { activeFilters: action.payload.activeFilters }
		});
