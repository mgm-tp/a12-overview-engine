...
import { padYear, padYearTime } from "../date-period-utils.js";
import type { OverviewModel } from "../../../../overview-model.js";
import { isDefined, formatRange, createDateFormatter } from "../../../../services/filter-format-utils.js";

import type { Resolver } from "./resolver.js";

export const yearResolver: Resolver<
	DateFilterState.YearViewInputState,
	...
	toLabel(criteria, range, { documentModel, getDateTimeFormat, fieldPath }) {
		...
		const labels = RangeCriteria.resolve(criteria, range)
			...
			.map(({ segment, value }) => {
				...
				return formatter.formatDate(padYear(value.value, { timezone, segment }), getDateTimeFormat({ kind: "year" }));
			});
