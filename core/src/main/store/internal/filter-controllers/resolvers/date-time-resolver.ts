...
export const dateTimeResolver: Resolver<DateTimeInput, string, string> = {
	accept(key) {
		return key === "dateTime";
	},
	...
	toLabel(criteria, range, { formatValue, fieldPath, subModel }) {
		const labels = RangeCriteria.resolve(criteria, range)
			.asValues()
			.map((state) => (isDefined(state?.value) ? formatValue({ value: state.value, fieldPath, subModel }) : null));

		return formatRange(...labels);
	},

	toQueryValue(input, _section, { documentModel, fieldPath }) {
