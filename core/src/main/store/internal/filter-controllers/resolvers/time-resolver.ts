...
export const timeResolver: Resolver<TimeInput, string, string> = {
	accept(key) {
		return key === "time";
	},
	...
	toLabel(criteria, range, { documentModel, getDateTimeFormat, fieldPath }) {
		...
		const labels = RangeCriteria.resolve(criteria, range)
			.asValues()
			.map((state) => formatter.formatDate(state?.value ?? null, getDateTimeFormat({ kind: "time" })));
...
export const standaloneTimeResolver: Resolver<TimeInput, string, string> = {
	...timeResolver,
	accept() {
		return true;
	}
