...
interface YearFragmentInput {
	readonly value: number | null;
	readonly error?: string;
}

export const fragmentYearResolver: Resolver<YearFragmentInput, string, string> = {
	accept(key) {
		return key === "year";
	},
	...
	toQueryValue(input) {
		return isDefined(input.value) ? String(input.value) : undefined;
	}
};
