...
export function getTargetDocumentModel(modelsState: ModelsState, modelId: string | undefined): DocumentModel {
	if (!modelId) {
		return modelsState.documentModel;
	}

	const targetDocumentModel = modelsState.subDocumentModels?.find((dm) => dm.header.id === modelId);

	return targetDocumentModel ?? modelsState.documentModel;
}
