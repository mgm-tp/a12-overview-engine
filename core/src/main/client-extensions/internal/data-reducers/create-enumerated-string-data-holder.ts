...
export const createEnumeratedStringDataHolderReducer: ActivityReducers.DataReducer = {
	reduce(dataHolders, action) {
		if (!OverviewEngineActions.createEnumeratedStringDataHolder.match(action)) {
			return dataHolders;
		}

		const enumeratedStringDataHolder: Activity.DataHolder<EnumeratedStringDataHolder.Data> = {
			descriptor: action.payload.descriptor,
			data: { ...action.payload.data, keyword: "" },
		...
		return [...(dataHolders ?? []), enumeratedStringDataHolder];
	}
};
