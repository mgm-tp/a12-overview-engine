...
/** @internal */
export const MultiSelectionActions = React.memo(function MultiSelectionActions() {
	...
	return (
		<>
			{showGroupDivider && <ActionBarGroupDivider />}
			{buttons.length > 0 && (
				<ButtonGroup>
					{buttons.map((button) => (
						<OverviewButton
							componentKey={OverviewModelKeys.MULTI_SELECTION}
							key={`multi-selection-action-${button.event}`}
							buttonModel={button}
