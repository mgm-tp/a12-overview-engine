...
/** @experimental until 40.0.0 - API may change without semver guarantees. */
export const EnumerationFilterSetting: FC<EnumerationFilterSettingProps> = memo(function EnumerationFilterSetting({
	state
}) {
	const { empty, invert } = state.options;

	const onOptionsChange = useDispatchFilterOptions<EnumerationFilterState>(state.model.id);
