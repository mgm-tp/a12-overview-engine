...
import { useAvailableDateFragmentPeriods } from "../../hooks/use-available-periods.js";
...
/** @experimental until 40.0.0 - API may change without semver guarantees. */
export const DateFragmentFilterSetting: FC<DateFragmentFilterSettingProps> = memo(function DateFragmentFilterSetting({
	state
}) {
	...
	const availablePeriods = useAvailableDateFragmentPeriods(state.model, state.element.fieldType.formatOfFragment);
				...
				<>
					{availablePeriods.length > 1 && (
						<FilterSettingSection label={localizedResource(t.period)}>
							<RadioSetting<OverviewModel.NewFilter.DateFragment.PeriodOption>
								name="date-fragment-filter-period"
