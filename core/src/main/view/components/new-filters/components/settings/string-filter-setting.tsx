...
import type { StringFilterState } from "../../../../../store/index.js";
...
/** @experimental until 40.0.0 - API may change without semver guarantees. */
export const StringFilterSetting: FC<StringFilterSettingProps> = memo(function StringFilterSetting({ state }) {
	const { empty, caseSensitive, exactMatch, invert } = state.options;
	...
	return (
		<>
			...
			{!(empty.enabled && empty.value) && (caseSensitive.enabled || exactMatch.enabled) && (
				<FilterSettingSection label={localizedResource(t.match)}>
					{caseSensitive.enabled && (
						<BooleanToggleSetting
							label={localizedResource(t.caseSensitive)}
							selectedValue={caseSensitive.value}
							onChange={(value) => onOptionsChange({ caseSensitive: { ...caseSensitive, value } })}
						/>
					)}
					{exactMatch.enabled && (
						<BooleanToggleSetting
							label={localizedResource(t.exactMatch)}
							selectedValue={exactMatch.value}
							onChange={(value) => onOptionsChange({ exactMatch: { ...exactMatch, value } })}
						/>
					)}
				</FilterSettingSection>
			)}
