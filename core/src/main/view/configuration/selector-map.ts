...
export const DefaultSelectorMap: SelectorMap = {
	attachmentThumbnail: (attachment) => {
		return (state) => attachmentThumbnailSelector(state, attachment?.attachment_id ?? undefined);
	}
};

const attachmentThumbnailSelector: (state: object, attachmentId: string | undefined) => string | undefined =
	createSelector(
		[
			(state: OverviewEngineContextType) => state.thumbnails,
			(_, attachment_id: string | null | undefined) => attachment_id
		],
