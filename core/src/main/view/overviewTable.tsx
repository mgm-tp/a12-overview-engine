...
function useRowEventHandlers(): RowEventHandlerGetter<JSONDocument> | undefined {
	...
	return React.useMemo(() => {
		const customEvent = overviewModel.content.defaultRowAction?.custom
			? overviewModel.content.defaultRowAction.event
			: undefined;
		...
		return ({ row, rowIndex }) => {
			...
			const collapsibleOption = overviewModel.content.configuration.multiSelection?.collapseOption;
			const selectionArea =
				overviewModel.content.configuration.multiSelection?.selectionArea ??
			...
			return {
				onClick: (event: React.MouseEvent<HTMLElement>) => {
					...
					if (
						(collapsibleOption === OverviewModel.MultiSelection.CollapseOption.COLLAPSIBLE_COLLAPSED ||
							collapsibleOption === OverviewModel.MultiSelection.CollapseOption.COLLAPSIBLE_EXPANDED) &&
						expandedMultiSelection
					) {
						selectRows(event, documentId);

						return;
					}

					if (collapsibleOption === OverviewModel.MultiSelection.CollapseOption.NON_COLLAPSIBLE && hasSelectedRow) {
						selectRows(event, documentId);
		...
		overviewModel.content.defaultRowAction?.custom,
		overviewModel.content.defaultRowAction?.event,
			...
			bodyCellRenderer: (props) => {
				const cellId =
					screenReaderColumnRef && props.column.columnModel?.id === screenReaderColumnRef.idref
						? toCellId(props.row.id, screenReaderColumnRef.idref)
						: undefined;
