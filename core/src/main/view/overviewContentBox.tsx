...
import { type ButtonPanel } from "./components/button-panel.js";
...
export const OverviewContentBox: React.ComponentType<OverviewContentBox.Props> = React.memo(
	function OverviewContentBox(props) {
		...
		const [showMobileSearchBar, setShowMobileSearchBar] = React.useState<boolean>(false);
...
export function useFooterBoxButtons(): ButtonPanel.ButtonType[] {
	...
	const mapOverviewButton = React.useCallback(
		(buttonElement: OverviewModel.ButtonElement, index: number, rank: "minor" | "major" = "minor") => ({
	...
	const minorButtons: ButtonPanel.ButtonType[] = React.useMemo(
		() => footerBox?.minorElements?.map((buttonModel, index) => mapOverviewButton(buttonModel, index)) ?? [],
		[footerBox?.minorElements, mapOverviewButton]
	);

	const majorButtons: ButtonPanel.ButtonType[] = React.useMemo(
		() => footerBox?.majorElements?.map((buttonModel, index) => mapOverviewButton(buttonModel, index, "major")) ?? [],
		[footerBox?.majorElements, mapOverviewButton]
	);

	return [...minorButtons, ...majorButtons];
}
