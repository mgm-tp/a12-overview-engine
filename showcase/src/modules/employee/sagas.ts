...
const SELECT_EMPLOYEE_EVENT = "selectEmployee";

export const EmployeeSagas = [rowClickSaga, createCancelButtonSaga("employee")];
...
function* rowClickSaga(): SagaGenerator<void> {
	yield* takeLatest(
		...
		function* (action: Action<OverviewEngineActions.EventPayload<Action<Events.RowClickedPayload>>>) {
			yield* call(handleRowClickWithCustomEvent, action, "employee", SELECT_EMPLOYEE_EVENT, "selectedEmployee");
		}
	);
}
