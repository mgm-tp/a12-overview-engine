...
test.describe.serial("Filter changes preserve column sorting", () => {
	...
	test("Applying a filter keeps the active sort direction", async ({ page }) => {
		...
		await header.click();
		const sortDirection = await header.getAttribute("aria-sort");

		if (sortDirection === null || sortDirection === "none") {
			throw new Error(`expected ascending/descending after click, got ${sortDirection}`);
		}
		...
		await expect(header).toHaveAttribute("aria-sort", sortDirection);
	});
});
