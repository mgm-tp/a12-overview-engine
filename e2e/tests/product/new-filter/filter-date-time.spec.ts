...
const FILTER_ID = "dateTimeField";

function makeDateTimeFilter(
	overrides: Partial<OverviewModel.NewFilter.DateTime.Item["options"]> = {}
...
function filterBarChip(page: Page) {
	return page.locator(`${Selector.FILTER_CONTENT}[title="[D] DateTime"]`);
}
...
test.describe.serial("DateTime Filter", () => {
	test.beforeAll(async ({ seed }) => {
		await seed("product");
	});

	test.describe("default config", () => {
		useModelOverride((model) => setFilterItems(model, [makeDateTimeFilter()]));

		test("Entering a dateTime range and applying narrows the table", async ({ page }) => {
			...
			const fromInput = dropdown.getByPlaceholder("MM/dd/yyyy hh:mm AM/PM").first();
			const toInput = dropdown.getByPlaceholder("MM/dd/yyyy hh:mm AM/PM").last();
			await fromInput.fill("01/01/2019 12:00 AM");
			await fromInput.blur();
			await toInput.fill("12/31/2019 11:59 PM");
			await toInput.blur();
	...
	test.describe("with empty preselected", () => {
		useModelOverride((model) => setFilterItems(model, [makeDateTimeFilter({ empty: { enabled: true, value: true } })]));
