...
test.describe.serial("Number Filter", () => {
	...
	test.describe("default config", () => {
		test("Entering a number value and applying filters the table", async ({ page }) => {
			...
			await dropdown.getByPlaceholder("Filter Value").fill("100");
		...
		test("Reset clears number filter", async ({ page }) => {
			await navigate(page, Showcase.PRODUCT_NEW_FILTER);
			await expect(page.getByRole("heading", { name: "List of all products (30)" })).toBeVisible();
	...
	test.describe("with empty preselected", () => {
		useModelOverride((model) =>
			updateNumberFilter(model, "number", (filter) => {
