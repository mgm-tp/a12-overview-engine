...
test.describe.serial("Filter Selector cancel (docked)", () => {
	useModelOverride((model) => setFilterItems(model, [makeNumberFilter(), makeBooleanFilter()]));
	...
	async function ensureSelectorOpen(page: Page) {
		...
		await expect(page.getByRole("heading", { name: "Available in stock" })).toBeVisible();
	}

	test("Editing a filter without Apply All leaves table count unchanged", async ({ page }) => {
	...
	test("Reset All discards pending edits without applying", async ({ page }) => {
		await navigate(page, Showcase.PRODUCT_NEW_FILTER);
		await ensureSelectorOpen(page);
		...
		await expect(inStockSection.getByRole("checkbox", { name: "Yes" })).not.toBeChecked();
	});
});
