...
test.describe.serial("Filter Selector header subtitle", () => {
	test.beforeAll(async ({ seed }) => {
		await seed("product");
	});

	test.describe("Without headerSubtitle", () => {
		...
		test("Only 'Filter' is shown as the title", async ({ page }) => {
	...
	test.describe("With headerSubtitle", () => {
		useModelOverride((model) =>
			overrideFilterConfiguration(model, {
				filterSelector: {
					// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
		...
		test("'Filter' subtitle above the modeled title", async ({ page }) => {
			await navigate(page, Showcase.PRODUCT_NEW_FILTER);
			await ensureSelectorOpen(page);
