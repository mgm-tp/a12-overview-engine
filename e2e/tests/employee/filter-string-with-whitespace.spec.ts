...
test.describe("String with whitespace", () => {
	...
	test.describe("normal string filter with enable_approximate_match_search = true", () => {
		...
		test.describe("filter a string field by a value contains whitespace", () => {
			test("filtered result should contain filter value", async ({ page }) => {
				const fullKeyword = "Manuel Simon";
				await page
				...
				await expect(page.locator(Selector.TABLE_BODY_ROW)).toContainText(fullKeyword);
				const partialKeyword = "Manuel Sim";
				await page.locator(Selector.BUTTON_OPEN_FILTER).click();
				await page
					.locator(Selector.FILTER_SELECTOR_LIST_ITEM)
					.locator(Selector.LIST_ITEM_TEXT, { hasText: "Full Name" })
					.click();
				await page.locator(Selector.INPUT_FILTER_VALUE).fill(partialKeyword);
				await page.locator(Selector.BUTTON_APPLY).click();
				await expect(page.locator(Selector.FILTER_SELECTOR)).not.toBeVisible();
				await expect(page.locator(Selector.TABLE_BODY_ROW)).toHaveCount(1);
				await expect(page.locator(Selector.TABLE_BODY_ROW)).toContainText(fullKeyword);
