...
test.describe("Date range", () => {
	...
	test("should work with start and/or end inputs", async ({ page }) => {
		...
		await page
			...
			.locator(Selector.LIST_ITEM_TEXT, { hasText: /^On Sale/ })
			.click();
		await inputByLabel(page, "Start Filter Value").fill("10/25");
		...
		await inputByLabel(page, "End Filter Value").fill("03/25");
